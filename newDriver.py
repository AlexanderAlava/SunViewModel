...
model = fasttext.load_model('model_1.bin', label_prefix='__label__')
...
while failed != 1:
	...
	if query[0].lower() == 'exit':
		break
	label = model.predict_proba(query)
	buket = label[0][0][0] 
	confidence = label[0][0][1]
	
	print '\nBuket Prediction: ',buket
	...
	# Confidence level is greater than 0.6
		...
		keyword = buket # This should be the first label
		print("Con > 0.6")
